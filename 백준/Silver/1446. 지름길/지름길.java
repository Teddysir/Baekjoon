...
public class Main {
	...
	static void dijkstra() {
		...
		while (!pq.isEmpty()) {
			...
			int temepTarget = temp.target;
			int tempWeigth = temp.weight;

			if (tempWeigth > dist[temepTarget]) {
				continue;
			}

			for (Node next : graph[temepTarget]) {
				int shortcutWeight = dist[temepTarget] + next.weight;
				if (shortcutWeight < dist[next.target]) {
					dist[next.target] = shortcutWeight;
					pq.add(new Node(next.target, shortcutWeight));
				}
			}
		}
	}
