...
//	static void solution(int start) { // 1, 2, 3, 4 구역 하나씩 시작
//
//		for (int i = 1; i <= N; i++) {
...
//	}
	...
	static void checker(boolean[] check) {
	    if (!isConnected(check, true) || !isConnected(check, false)) return;
	...
	static boolean isConnected(boolean[] check, boolean flag) {
	    ...
	    if (q.isEmpty()) return false; // 해당 그룹에 구역 없음
